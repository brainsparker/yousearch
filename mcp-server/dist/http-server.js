...
async function search(query) {
    ...
    const response = await fetch(`https://ydc-index.io/v1/search?query=${encodeURIComponent(query)}`, {
 ...
 */
function formatResultsAsText(results, demoMode) {
    const formatted = [];
    if (demoMode) {
    ...
    if (results.results?.web) {
        ...
        results.results.web.slice(0, 10).forEach((result, idx) => {
            ...
            if (result.description)
                formatted.push(`   Description: ${result.description}`);
